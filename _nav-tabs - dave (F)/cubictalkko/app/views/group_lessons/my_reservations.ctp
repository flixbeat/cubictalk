...
<div id = "wrapper"  class = "div-container">
	...
	<table class = "table">
		...
		<tr>
			...
			<td><button class = "btn btn-warning" onclick = "cancelClass(<?=$userId?>,<?=$groupLesson['group_lessons']['id'];?>,<?=$groupLesson['lessons']['id'];?>)"><?=$language[$session->read('lang')]['btn_cancel']?></button></td>
		</tr>
		<?php endforeach;?>
		<?php
